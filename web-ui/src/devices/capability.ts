...
export interface CapabilityDef<TConfig = any> {
  ...
  /**
   * Declares the absolute DMX channel layout for this capability.
   * Channel numbers are read from the capability's own config (1-based `ch` field).
   * Omit entirely for capabilities with no DMX output (name, transform, tilt, power…).
   */
...
/** Type-safe factory: pairs a capability with its config. */

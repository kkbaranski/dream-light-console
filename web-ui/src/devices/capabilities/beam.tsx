...
export function findBeamOriginNode(
  ...
  if (tiltBound) {
    return getCachedNode(model, (tiltBound.config as { modelNode: string }).modelNode);
  }

  const panBound = caps.find((b) => b.cap.type === "pan");
  if (panBound) {
    return getCachedNode(model, (panBound.config as { modelNode: string }).modelNode);
  }
